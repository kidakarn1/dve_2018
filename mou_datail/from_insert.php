...
$major=$_POST['major'];
        ...
        if(isset($add)=="true"){
            $select_count="select * from mou_detail";
            $res_count=mysqli_query($conn,$select_count);
            $count=$row_count=mysqli_num_rows($res_count);
            $count=$count+1;
            echo $select_mou="select * from mou where business_id = '{$business}'";
            $res_mou=mysqli_query($conn,$select_mou);
            $row_mou=mysqli_fetch_assoc($res_mou);
            $mou_idd=$row_mou["mou_id"];
            $add="insert into mou_detail values('$count','$mou_idd','$education','$major')";
    ...
    ?>
        ...
        <div class="col-sm-3">
                ...
                <?php while($row_business=mysqli_fetch_assoc($res_business)){?>
	                <option value="<?php echo $row_business["business_id"];?>"><?php echo $row_business["business_name"];?>
                <?php }?>
                </select>
                </div>
        </div>
        <div class="form-group">
            ...
            <div class="col-sm-3">
                ...
                <?php while($row_education=mysqli_fetch_assoc($res_education)){?>
	                <option value="<?php echo $row_education["edu_name"];?>"><?php echo $row_education["edu_name"];?>
                <?php }?>
                </select>
                </div>
        </div>
        <div class="form-group">
            ...
            <select name="major" class="form-control">
                <?php while($row_major=mysqli_fetch_assoc($res_major)){?>
	                <option value="<?php echo $row_major["major_id"];?>"><?php echo $row_major["major_name"];?>
                <?php }?>
                </select>
                </div>
                </div>
                <div class="text-center"> 
                <input class="btn btn-md btn-success" type="submit" name="add" value="บันทึก"></th></tr>
                </div>
        </form>
<?php
