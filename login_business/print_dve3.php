...
<html lang="en">
    <head>
		...
		@SESSION_START();
			...
			$connect=$conn;
            $id=$_GET['id'];
            $sql_check_std_id = "select * from student where std_id = '{$id}'";
            $res_check_std_id = mysqli_query($conn,$sql_check_std_id);
            $row_check_std_id = mysqli_fetch_assoc($res_check_std_id);
            $sql_student="";
            if ($row_check_std_id["system_id"]=="1"){
			    $sql_student="select * from student,major,district,amphur,province,groups,teacher where student.std_id='{$id}' and 
                ...
                groups.teacher_id = teacher.teacher_id";
            }
            else {
                $sql_student="select * from student,major,district,amphur,province,groups,teacher where student.std_id='{$id}' and 
                student.major_id = major.major_id and 
    ...
    <body>
        ...
        <table>
            <tr>
				...
				<tr><td>ชั้น <?php if ($row_student['edu_id'] == 2) {echo " ปวช";
	$level1=$row_student['edu_id'];} ?>
	 <?php if ($row_student['edu_id'] == 3) {echo "ปวส";
	$level1=$row_student['edu_id'];} ?> 
	 ปีที่ <?php if ($row_student['edu_id'] == 2){
					if ($level > 3){
						echo "3";
						}
			echo $level;
		}
		if ($row_student['edu_id'] == 3){
			if ($level > 2){
				echo "2";
				}
				echo $level;
...
<?php if ($row_student['system_id'] == 1) {echo "ระบบปกติ";} ?>  <?php if ($row_student['system_id'] == 2) {echo "ระบบทวิภาคี";} ?>  	
                ...
                <tr><td> <?php echo $row_student['genius'] ?></td></tr>
               ...
               <tr><td><h4>๗.ครูที่ปรึกษา  <?php echo  $row_student['teacher_name'];?></h4></td></tr>
			   </td></tr>
