...
$res_training = select_training($conn);
?>
                    ...
                    <center>
                        <table class="table">
                            ...
                            while ($row_training = mysqli_fetch_assoc($res_training)) {
                                ...
                                <tr>
                                    <td><?php echo $i; ?></td>
                                    <td><?php echo $row_training["head_name_std"] . " " . $row_training["std_name"] ?></td>
                                    <td><?php echo $row_training["phone"] ?></td>
                                    <td><?php echo $row_training["dep_name"] ?></td>
                                    <td><?php echo $row_training["business_name"] ?></td>
                                    <td><a href="menu_dve.php?id=<?php echo $row_training["citizen_id"] ?>">จัดการ</a></td>
                                </tr>
                                <?php
                                $i++;
                            }
...
function select_training($conn) {
    $sql = "select * from training_dve,student,business,department,minor,teacher where "
            . "training_dve.citizen_id = student.citizen_id and "
            . "training_dve.business_id = business.business_id and "
            . "minor.dep_id = department.dep_id and "
            . "training_dve.teacher_id = teacher.teacher_id and "
            . "training_dve.training_status = 'อื่นๆ'";
