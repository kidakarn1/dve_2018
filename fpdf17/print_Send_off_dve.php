...
$sql_bus = "select * from training_dve,business,student,major,department 
where training_dve.citizen_id = '$business_id' 
and  training_dve.business_id = business.business_id 
and training_dve.citizen_id = student.std_id
...
$pdf->SetFont('angsa', '', 16);
			$pdf -> SetLeftMargin(20);
			$pdf ->Cell(283,7,iconv( 'UTF-8','TIS-620',$row_b['business_name']), 0 ,1,"C");
		  ...
		  $pdf->Cell(135, 7, iconv('UTF-8', 'TIS-620', '( ฝึกงานระหว่าง วันที่'."  ".dateThai($row_b['start_date'])." ".'ถึง'."  ".dateThai($row_b['end_date'])." ".')'), 0, 1, "C");
		   ...
		   'สาขาวิชา'.$row_b['major_name']." ".'จำนวน '.$nub." ".'คน'." ".'ดังนี้'), 0, 1, "C");
			...
			$pdf->Ln(5);
			$pdf->Image('4.jpg',28,117,7,0,'','http://www.chontech.ac.th/');
