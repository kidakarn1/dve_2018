...
<?php
			...
			$pdf->Cell(260, 9, iconv('UTF-8', 'TIS-620', 'หนังสือรับรองการฝึกอาชีพ'), 0, 1, "C");
		   ...
		   $pdf->Cell(260, 10, iconv('UTF-8', 'TIS-620', 'โดยได้รับการฝึกอาชีพใน'), 0, 1, "C");
		   $pdf->SetFont('angsa', '', 30);
		   $pdf->Ln(1);
		  ...
		  $pdf->Cell(250, 8, iconv('UTF-8', 'TIS-620',  $row_school["deputy_academic"]), 0, 1, "C");
		   ...
		   $pdf->Cell(250, 8, iconv('UTF-8', 'TIS-620', ' ผู้อำนวยการ'.$row_school["school_name"]), 0, 1, "C");
...
<?php function select_b($id,$connect){
	 $sql="select * from training_dve,business,minor,student,major
	 	   where training_dve.training_status = 'อนุมัติสถานประกอบการ' 
           and training_dve.business_id = business.business_id and
	       training_dve.minor_id = minor.minor_id and 
	       training_dve.citizen_id = student.citizen_id and 
	       student.citizen_id='$id'  and 
