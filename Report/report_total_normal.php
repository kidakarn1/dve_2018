...
<html lang="en">
 ...
 <body>
	...
	<br>
	รายงานสรุปจำนวนนักเรียนออกฝึกงาน <br>
	ระหว่างวันที่ <?php echo dateThai($rowNH["start_date"])?> ถึง  <?php echo dateThai($rowNH["end_date"])?>  ภาคเรียนที่<?php echo $rowY["term"]?> ปีการศึกษา  <?php echo $rowY["inter_year"];?>
		...
		<table border="1">
			...
			<tr>
				...
				$sqlCount = "select * from business,training_normal,student where business.business_id = training_normal.business_id 
...
<?php
	...
	function get_bus($conn) {
		$sql = "select * from business,training_normal,student where business.business_id = training_normal.business_id 
    and training_normal.citizen_id = student.citizen_id
    and training_normal.normal_status != 'เสร็จสิ้นการฝึก'
		group by training_normal.business_id";
		$res = mysqli_query($conn,$sql);
		return $res;
	}
?>
